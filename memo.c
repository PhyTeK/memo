/* 
   ...
   The grade can only be: G (three working functions: -h -i -a) or IG.
...
int main(int argc, char **argv ){
  ...
  sprintf(dtm,"%d-%d-%d %2d:%2d:%2d", tm.tm_mday, \
  ...
  case 'h':
    ...
    break;
  case 's':
    search_data(DEFAULT,argv[2]);
    break;
  case 'a':
    append_data(DEFAULT,dtm,argv[2]);
    break;
  case 'd':
    delete_data(DEFAULT,strtol(argv[2],'\0',10));
    break;
  default:
    puts("Unknown option!");
    goto start;
